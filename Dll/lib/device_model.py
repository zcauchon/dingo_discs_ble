...
class DeviceModel:
    ...
    def openDevice(self):
        ...
        try:
            ...
            print("Error Opening Device:" + self.serialConfig.portName + str(self.serialConfig.baud))
    ...
    def closeDevice(self):
        ...
        :return: 无返回
        """
        if self.serialPort is not None or self.isOpen:
    ...
    def decToHex(self, num):
        ...
        :return:
        """
        tempStr = str(str(hex(num).upper()[2:]))
        if (len(tempStr) % 2!=0):
            tempStr = "0" + tempStr
        return tempStr
    ...
    def readReg(self, regAddr, regCount, waitTime):
        """
        读取寄存器
        :param regAddr: 寄存器地址
        ...
        """
        if self.protocolResolver is not None:
            return self.protocolResolver.readReg(regAddr, regCount, waitTime, self)
        else:
            return False
